...
Promise.prototype.then = function (onFulfilled, onRejected) {
  ...
  // 看当前 promiseA 的状态，如果不是 PENDING，就将 callback 和 promiseB 放到微循环队列中，等待执行，否则，将 callback 和 promiseB 放在 promiseA 的队列中。这里很关键，起到了前后 promise 连接的作用。
...
在 handlers.resolve 中，我们可以通过 promiseB 对象，在它的 queue 里面，找到下一个 promsieC 和对应的 callback 继续执行。
```javascript
...
```
this.promise 是下一个 promise，this.onFulfilled 是下一个 callback。这样又进入到了微循环队列中，等到执行。
...
```
ok，大家可以调试试一下，不过执行顺序可能会比较绕。reject 和 resolve 是差不多的，希望大家能举一反三，这里就不继续介绍了。

## 总结
这里简单的总结一下
- then 的返回值大部分情况下是一个新的 promise 对象，也可能返回自身
